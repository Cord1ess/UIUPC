...
const DetailItem = ({ label, value, fullWidth = false, isLink = false }: { label: string; value: any; fullWidth?: boolean; isLink?: boolean }) => (
  <div className={`flex flex-col gap-2 p-5 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl border border-black/[0.03] dark:border-white/[0.03] hover:border-uiupc-orange/20 transition-all group ${fullWidth ? 'md:col-span-2' : ''}`}>
    ...
    {isLink && value && value !== "Not provided" && value !== "N/A" ? (
      ...
      >
        {value} <FaExternalLinkAlt className="text-[10px] shrink-0" />
      </a>
    ) : (
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 md:p-10">
          ...
          <motion.div 
            ...
            className="relative w-full max-w-5xl bg-white dark:bg-[#0a0a0a] rounded-[3rem] border border-black/10 dark:border-white/10 shadow-3xl flex flex-col overflow-hidden max-h-[90vh]"
          >
            {/* Premium Header */}
            <div className="relative p-8 md:p-12 border-b border-black/5 dark:border-white/5 bg-zinc-50/50 dark:bg-zinc-900/30 flex flex-col md:flex-row md:items-center justify-between gap-8">
              <div className="flex items-center gap-8">
                <div className="relative group">
                  <div className="w-24 h-24 rounded-[2.5rem] bg-zinc-950 dark:bg-white flex items-center justify-center text-white dark:text-zinc-900 text-4xl font-black shadow-2xl shadow-black/20 group-hover:scale-105 transition-transform duration-500">
                    {name.charAt(0)}
                  </div>
                ...
                <div className="space-y-2">
                  <span className="px-3 py-1 bg-uiupc-orange/10 text-uiupc-orange text-[9px] font-black uppercase tracking-widest rounded-lg border border-uiupc-orange/20">
                    {dataType === "membership" ? "Member Details" : "Photo Submission"}
                  </span>
                  ...
                  <div className="flex flex-wrap items-center gap-6 text-[10px] font-black uppercase tracking-widest text-zinc-500 pt-1">
                    <span className="flex items-center gap-2 group cursor-pointer hover:text-uiupc-orange transition-colors"><FaEnvelope className="text-uiupc-orange group-hover:scale-110 transition-transform" /> {email}</span>
                    {phone && <span className="flex items-center gap-2 group cursor-pointer hover:text-uiupc-orange transition-colors"><FaPhone className="text-uiupc-orange group-hover:scale-110 transition-transform" /> {phone}</span>}
                  </div>
                </div>
              </div>
              <button 
                onClick={onClose} 
                className="absolute top-8 right-8 md:static w-14 h-14 flex items-center justify-center rounded-2xl bg-white dark:bg-zinc-800 text-zinc-400 hover:text-red-500 hover:bg-red-500/5 transition-all shadow-sm border border-black/5 dark:border-white/5"
              >
                <FaTimes size={20} />
              </button>
            </div>
            ...
            {/* Scrollable Content */}
            <div className="p-8 md:p-12 overflow-y-auto flex-1 space-y-12 custom-scrollbar">
              {dataType === "membership" ? (
                <>
                  <DetailSection title="Basics" icon={FaUser}>
                  ...
                  <DetailSection title="Social & Interests" icon={FaFacebook}>
                  ...
                  <DetailSection title="Payment & Schedule" icon={FaCreditCard}>
                  ...
                  <DetailSection title="Message" icon={FaCommentAlt}>
                    <div className="md:col-span-2">
                      <div className="relative p-8 bg-uiupc-orange/5 border border-uiupc-orange/15 rounded-[2.5rem] shadow-sm overflow-hidden group">
                        <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-opacity">
                          <FaCommentAlt size={40} className="text-uiupc-orange" />
                        </div>
                ...
                <>
                  <DetailSection title="Submission" icon={FaCalendarAlt}>
                    <DetailItem label="Submitted On" value={timestamp} />
                  ...
                  <DetailSection title="Photo Details" icon={FaLink}>
            ...
            {/* Footer Control */}
            <div className="p-8 md:p-10 border-t border-black/5 dark:border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-zinc-50/50 dark:bg-zinc-900/50">
              <div className="flex flex-col">
                <span className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Reference</span>
                <span className="text-[10px] font-mono text-zinc-500">ID: {item.id || "N/A"}</span>
              </div>
              <button 
                onClick={onClose} 

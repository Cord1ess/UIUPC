...
## 6. Module Architecture (V2.0 Core)

To ensure long-term maintainability and resolve module-resolution ambiguities common in large Next.js projects, we have moved to a strict Named Export strategy.

### Rule 1: Named Exports Over Default Exports
Always export components by name. This prevents IDE resolution errors and ensures that components are imported consistently across every file.
```tsx
// Correct
...
### Rule 2: Barrel-Indexing (The Component Entry Point)
Every component directory should contain an `index.ts` file that re-exports its children. This creates a clean, unified import API for features.
```tsx
...
### Rule 3: Destructured Feature Imports
Imports from Feature modules should always be destructured from the base directory path, shortening import lists and improving readability.
```tsx
import { Sidebar, Dashboard, Modal } from "@/features/admin/components";
```

        ...
        {activeToolId === "cropper" ? (
          ...
          >
             ...
             >
                ...
                {activeImage.edits.editorial?.vignette > 0 && (
                  <div 
                    className="absolute inset-0 pointer-events-none"
                    style={{
                      background: `radial-gradient(circle, transparent 20%, rgba(0,0,0,${activeImage.edits.editorial.vignette / 100}) 100%)`
                    }}
                  />
                )}

                {/* Real-time Temperature Preview Overlay */}
                {activeImage.edits.editorial?.temperature !== 0 && (
                  <div 
                    className="absolute inset-0 pointer-events-none"
                    style={{
                      backgroundColor: activeImage.edits.editorial.temperature > 0 
                        ? `rgba(255, 140, 0, ${Math.abs(activeImage.edits.editorial.temperature) / 300})`
                        : `rgba(0, 100, 255, ${Math.abs(activeImage.edits.editorial.temperature) / 300})`,
                   ...
                   >
                     {watermark.tiled ? (
                        <div 
                          ...
                          style={{
                            backgroundImage: watermark.type === 'logo' ? `url(${watermark.imageUri})` : 'none',
                            backgroundRepeat: 'repeat',
                            backgroundSize: watermark.type === 'logo' ? `${watermark.scale * 100}cqw` : 'auto',
                            display: 'flex',
                            flexWrap: 'wrap',

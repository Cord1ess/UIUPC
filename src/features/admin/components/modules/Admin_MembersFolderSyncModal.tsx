...
import { IconFolder, IconClose, IconSync, IconCheckCircle, IconExclamationCircle, IconUserCircle } from '@/components/shared/Icons';
...
interface Admin_MembersFolderSyncModalProps {
  isOpen: boolean;
  onClose: () => void;
  currentYear: string;
...
export const Admin_MembersFolderSyncModal: React.FC<Admin_MembersFolderSyncModalProps> = ({
  ...
  const { data: members } = useSupabaseData("members", { 
    filters: currentYear !== 'all' ? { session: currentYear } : {},
    limit: 1000
  });
  ...
  useEffect(() => {
    if (driveFiles.length > 0 && members.length > 0) {
      const newMatches = members.map(m => {
        ...
        const mName = normalize(m.full_name);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      for (const update of updates) {
        await executeAdminMutation('members', 'update', { image_url: update.image_url }, update.id);
      }

      alert(`Successfully updated ${updates.length} member photos!`);
      onSuccess();
      onClose();
    } catch (err) {
          ...
          >
            <div className="p-8 md:p-12 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] mt-1">Session Year: {currentYear === 'all' ? 'All Sessions' : currentYear}</p>
              </div>
                      ...
                      <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-5 bg-white dark:bg-zinc-900/30 rounded-2xl border border-zinc-200 dark:border-zinc-800 group hover:border-uiupc-orange/30 transition-all gap-4">
                        <div className="flex items-center gap-4">
                           ...
                           <div className="flex flex-col min-w-0">
                             <span className="text-xs font-black dark:text-white uppercase truncate">{m.member.full_name}</span>
                             <span className="text-[9px] font-bold text-zinc-500 uppercase truncate">{m.member.department} • {m.member.session}</span>
                           </div>
                        </div>
